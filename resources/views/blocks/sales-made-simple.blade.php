...
<div class="h-full relative transform">
  <h1 class="z-20 text-4xl tablet:text-5xl text-center text-white font-bold h-full flex flex-col justify-center items-start tablet:items-center pointer-events-none">
  ...
  <aside class="absolute w-full top-0 left-0 h-full text-white flex items-center z-50 pointer-events-none">
    <div class="transform -rotate-90 -translate-y-1/2 translate-x-1/2 origin-center whitespace-nowrap inline-flex gap-32 justify-start absolute right-2 items-center">
      ...
      <a class="pointer-events-all cursor-pointer" href="mailto:{{ get_field('company_e-mail_address', 'options') }}">{{ get_field('company_e-mail_address', 'options') }}</a></div>
  </aside>
</div>
